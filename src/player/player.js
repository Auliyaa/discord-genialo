...
class player
{
  ...
  // return the proper downloader for a given url, based on the url provider
  downloader(url)
  {
  ...
  async handle_play(str, message)
  {
    ...
    let len = this.genialo.voice.push(voice_channel, r.results[0].title, this.downloader(r.results[0].url),
                () => {
  ...
  handle_choose(str, message)
  {
    ...
    this.current_search = null;
    let len = this.genialo.voice.push(voice_channel, p.title, this.downloader(p.url),
                () => {
